...
- [x] Model the raw transactional session protocol and expected unsafe mutants
  ...
  TASK 3 EVIDENCE: Pinned TLC v1.7.4 exhaustively generated 1,029 raw successor states, reached 476 distinct raw states, and emitted 1,028 labelled edges in 1 second using one worker, below both reviewed ceilings. Online and offline runs passed all named invariants and terminal-aware deadlock safety. Each of the six expected-failure controls exited nonzero with its exact invariant and action anchor. Complete logs, metadirs, and action-labelled DOT graphs remain under ignored `.build/tla/session/model/`; the model contains no simulation, state constraint, fairness condition, or `Claimed` status.
...
- [x] Route reconcile, stop/signal authorization, and terminal lifecycle decisions through the reducer
  ...
  TASK 8 EVIDENCE: Dead/token-mismatch reconcile now starts a reducer teardown, executes ordered reap/socket effects, and only then classifies `TeardownProven`. Stop preserves revoke → immediate identity recheck → reducer authorization → bare/group signal → socket → ordered reap → terminal commit; nil/stale/tokenless and already-stopped revoke behavior remains pinned. Finish, Stop, and reconcile share reducer-owned terminal projection while preserving failure metadata, commit uncertainty, and the historical concrete `Detached` byte. The pre-existing decodable stale-Created Stop signal lane was restored explicitly outside the theorem after independent review caught the off-limits change. `Remove` is untouched. The mutation baseline shrank from 119 entries/45 owners to 103 entries/42 owners; focused, race, model/mutant, graph, and mutation gates exited 0.
...
- [x] Route running-session widen through the reducer
  ...
  TASK 10 EVIDENCE: Running widen now uses one serialized reducer driver for durable upper-bound candidate commit, separately classified apply and positive inspect, and final applied-generation commit. A commit-outcome matrix covers initial/final known-new, known-old, and both-world unknown handling; an uncertain first commit with unproven teardown retains the pending durable superset and transition instead of selecting old. Apply success without matching inspect fails closed. Duplicate/non-running, metadata, persistent authority, legacy bootstrap, tokenless owner framing, concurrency, fixed errors, and lock duration remain pinned. The default-engine regression proves the former recovery/Ensure inspection sequence, force-recreate override argv, and override-based readiness/hash ACK byte-for-byte in order. Independent review's two high findings were closed. The mutation baseline shrank from 103 entries/42 owners to 101/42; focused, race, full Go, model/mutant, graph, mutation, and diff gates exited 0.

- [x] Route running-session narrow through the reducer
  ...
  TASK 11 EVIDENCE: Running revoke now derives durable intent, narrower apply, positive inspect, and final shrink candidates from the reducer/adapter. The outcome matrix covers intent known/unknown, final known-new/known-old/both-world unknown, and proven/unproven teardown. Known apply/inspect failure and known-old final commit re-ACK the old runtime before clearing intent; unknown final durability never restores wider runtime. Unproven intent uncertainty retains the safe old upper bound plus intent marker. Duplicate/not-found/non-running, revisions, persistent authority, lock serialization, and the historical Running-revoke `UpdatedAt` frame are pinned. Independent review withdrew its metadata finding after comparison with pre-refactor dataflow. The mutation baseline shrank from 101 entries/42 owners to 94/40; focused, race, full Go, model/mutant, graph, mutation, and diff gates exited 0.

- [x] Route persisted recovery and fail-closed teardown through the reducer
  ...
  TASK 12 EVIDENCE: Persisted widen/narrow markers now enter adapter-bound active/recovery states; exact old/new inspect results are correlated without guessing, already-applied generations avoid replacement, unknown narrow runtime restores only the durable old generation through the existing Ensure+ACK recovery action, and legacy bootstrap records canonical applied bytes only after apply/inspect proof. Recovery commit failures fail closed. Network uncertainty now requests reducer teardown; synchronous managed container/network reap is the proof boundary (not SIGTERM alone), only `TeardownProven` projects Stopped/empty runtime, and `TeardownUnknown` persists the adapter-owned fixed marker while retaining the selected durable upper bound/transition with bounded commit retries. Review-driven synthetic recovery state and direct CLI marker writes were removed; exact pre-crash NarrowApply replay remains on the reachable relation. The mutation baseline shrank from 94 entries/40 owners to 80/36. Focused, race, full Go, model/mutant, graph, mutation, and diff gates exited 0.
...
- [x] Run independent model/spec/code review and authoritative verification
  ...
  FINAL ACCEPTANCE (2026-07-18): Three isolated review lanes covered the formal relation/invariants, production reducer/adapter routing and frozen behavior, and checker/parser/comparator/mutation/CI/dependency tooling; no blocker/high remained. Pinned TLC v1.7.4 generated 1,029 successor states, reached 476 distinct raw states, and emitted 1,028 labelled raw edges in 1 second on both online and `TLA_OFFLINE=1` runs. All six mutants failed for their exact named invariant and action anchor. The epistemic quotient produced 261 normalized states and 559 labelled edges, equal to the Go graph in both directions, with all 28 labels and every action/outcome/effect category non-vacuously covered. The final mutation allowlist is 80 entries across 36 owners.

  The exact authoritative command exited 0: online/offline model+graph+mutation checks, shuffled `./...`, full session/container/CLI race tests, `make check` (including 242 Emacs tests: 241 expected passes and one expected Doom-shim skip), and static `make build`. The supported local evidence host was Darwin arm64 with Go 1.26.4, OpenJDK 26.0.1, and Emacs 32.0.50; CI separately pins Temurin 21 on GitHub and requires Java 17+ on the native Darwin/arm64 Woodpecker host. The shipped command dependency closure contains no TLA/Java package, and final status before this acceptance note was the clean `0116-tla-session-boundary` branch.

  Acceptance remains bounded: it proves the named raw-state safety invariants and terminal-aware deadlock property only for one record, two symbolic owners/grants, and revisions 0..2. It does not prove fairness/liveness, arbitrary revisions/grants, SHA-256, POSIX durability, kernel process liveness, Docker/Squid behavior, external runtime tampering, tokenless legacy identity, CUE/public contracts, credentials, workspace/mount safety, or teardown implementations beyond their explicit effect contracts. Any future characterization conflict or newly discovered behavior defect requires a separate approved RED→GREEN change; it must not be hidden by changing the model or quotient.
