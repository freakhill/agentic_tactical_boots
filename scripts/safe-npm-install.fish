...
function __safe_npm_help
    echo "safe-npm-install — strict npm install wrapper"
    echo ""
    echo "Description:"
    ...
    echo "  scripts/sandboxctl.fish safe-npm"
    echo ""
    echo "Notes:"
...
function safe-npm-install --description "Install npm dependencies with safer defaults"
    ...
    if not test -f package-lock.json
        echo "Error: package-lock.json is required for safe-npm-install (run from the project root)." 1>&2
        echo "" 1>&2
    ...
    if test (count $argv) -gt 0
        echo "Error: safe-npm-install does not accept package names; it installs from the lockfile only." 1>&2
        echo "" 1>&2
        __safe_npm_help_to_stderr
        return 1
    end
