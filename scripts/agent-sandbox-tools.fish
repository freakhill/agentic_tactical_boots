...
function __agent_sandbox_tools_examples
    ...
    echo '  agent-sandbox-tools shell'
    echo '  scripts/sandboxctl.fish docker-tools shell'
    # END AUTOGEN: examples
end

function __agent_sandbox_tools_help
    echo "agent-sandbox-tools — Docker stack runner for the 'agent-tools' container"
    echo ""
    echo "Description:"
    echo "  Same UX as agent-sandbox but targets the tool-preinstalled image"
    ...
    echo "Usage:"
    echo "  source scripts/agent-sandbox-tools.fish"
    echo "  agent-sandbox-tools run [options] [command ...]"
    echo "  agent-sandbox-tools shell [options]"
    echo "  agent-sandbox-tools up"
    echo "  agent-sandbox-tools down"
    echo "  agent-sandbox-tools tui                 (interactive launcher; requires gum)"
    ...
    echo "  - For the bare agent without preinstalled tools, use 'agent-sandbox' instead."
...
function __agent_sandbox_tools_tui
    if not command -sq gum
        ...
        echo "" 1>&2
        echo "Or use the CLI directly. See: agent-sandbox-tools help" 1>&2
        return 1
    end

    while true
        echo ""
        gum style --bold --foreground 212 "agent-sandbox-tools — Docker stack ('agent-tools' service)"
        ...
        switch "$choice"
            case "Bring the stack up*"
                __agent_sandbox_tools_show_cli "agent-sandbox-tools up"
                if gum confirm --default=true "Build agent-tools + start proxy?"
                    agent-sandbox-tools up
                end
            case "Open a one-shot*"
                __agent_sandbox_tools_show_cli "agent-sandbox-tools shell --network-policy $policy"
                if gum confirm --default=true "Open shell in agent-tools container?"
                    agent-sandbox-tools shell --network-policy "$policy"
                end
            case "Run a one-off*"
                ...
                __agent_sandbox_tools_show_cli "agent-sandbox-tools run --network-policy $policy $cmd"
                if gum confirm --default=true "Run '$cmd' in agent-tools container?"
                    agent-sandbox-tools run --network-policy "$policy" $cmd
                end
            case "Bring the stack down"
                __agent_sandbox_tools_show_cli "agent-sandbox-tools down"
                if gum confirm --default=false "Stop and remove the stack?"
                    agent-sandbox-tools down
                end
            case "Quit"
                return 0
        end
