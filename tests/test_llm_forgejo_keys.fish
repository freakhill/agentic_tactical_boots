...
function test_help_advertises_here_and_tui
    set -l out (__invoke --help)
    assert_contains "llm-forgejo-key help mentions here" "$out" "here create-pair"
    assert_contains "llm-forgejo-key help mentions tui" "$out" "llm-forgejo-key tui"
end

function test_here_requires_subcommand
    set -l out (__invoke here)
    set -l rc $status
    assert_eq "llm-forgejo-key here no-sub fails" $rc 1
    assert_contains "llm-forgejo-key here no-sub message" "$out" "requires a subcommand"
end
...
function test_here_outside_git_repo_fails
    ...
    set -l rc $status
    assert_eq "llm-forgejo-key here outside-repo fails" $rc 1
    assert_contains "llm-forgejo-key here outside-repo message" "$out" "could not infer repo"
end
...
function test_here_no_matching_instance_profile_fails_clearly
    ...
    set -l out (command fish -c "$body" 2>&1)
    set -l rc $status
    assert_eq "llm-forgejo-key here no-instance fails" $rc 1
    assert_contains "llm-forgejo-key here no-instance message" "$out" "no Forgejo instance profile"
    assert_contains "llm-forgejo-key here suggests instance-set" "$out" "instance-set"
end
...
function test_here_repo_inference_url_forms
    for url in \
        ...
        assert_contains "llm-forgejo-key infer host from $url" "$out" "forgejo.example.com"
        assert_contains "llm-forgejo-key infer repo from $url" "$out" "owner/repo"
    end
end
...
function test_tui_without_gum_prints_install_hint
    ...
    set -l rc $status
    assert_eq "llm-forgejo-key tui no-gum fails" $rc 1
    assert_contains "llm-forgejo-key tui no-gum mentions gum" "$out" "gum"
    assert_contains "llm-forgejo-key tui no-gum suggests brew install" "$out" "brew install gum"
end
